...
# import numpy as np
from google.cloud import automl_v1beta1 as automl_interface
...
def get_gcp_credentials():
    """Centralized GCP authentication for all services."""
...
def get_storage_client():
    """Returns authenticated Cloud Storage client."""
    return storage.Client(credentials=get_gcp_credentials())


def get_bigquery_client():
    """Returns authenticated BigQuery client."""
    return bigquery.Client(credentials=get_gcp_credentials(),
                           project=get_gcp_credentials().project_id)
...
def test_authentication():
    ...
    try:
        ...
        print("✅ BigQuery connected | Project:", bq.project)

        storage = get_storage_client()
        print("✅ Cloud Storage connected | Project:", storage.project)
...
def imagenet_to_gcloud(kettle, clean_uid, bucketname, format, dryrun=False):
   ...
   # from gcloud import get_storage_client
    ...
    with open(csv_file, 'w') as train_file:
        for split, dataset in zip(['TRAIN', 'TEST'], [kettle.trainset, kettle.validset]):
            for sample, label, index in dataset:
                ...
                filename = f'img_temp/temp_image_{index}{format}'
...
def create_dataset_automl(uid, project_id, multilabel=False, wait_for_response=True):
    ...
    client = automl_interface.AutoMlClient()
    ...
    if multilabel:
        classificationtype = automl_interface.enums.ClassificationType.MULTILABEL
    else:
        classificationtype = automl_interface.enums.ClassificationType.MULTICLASS
    metadata = automl_interface.types.ImageClassificationDatasetMetadata(
        classification_type=classificationtype
    )
    print(metadata)
    dataset = automl_interface.types.Dataset(
...
def upload_to_automl(dataset_id, project_id, csvpath, wait_for_response=True):
    ...
    gcs_source = automl_interface.types.GcsSource(input_uris=input_uris)
    input_config = automl_interface.types.InputConfig(gcs_source=gcs_source)
...
def train(dataset_id, display_name, project_id, dryrun=False, edge=True):
    ...
    if edge:
        metadata = automl_interface.types.ImageClassificationModelMetadata(
    ...
    model = automl_interface.types.Model(
        display_name=display_name,
...
def predict(model_id, project_id, csvpath_test, resultpath_cloud):
    ...
    prediction_client = automl_interface.PredictionServiceClient()
    ...
    gcs_source = automl_interface.types.GcsSource(input_uris=[input_uri])

    input_config = automl_interface.types.BatchPredictInputConfig(gcs_source=gcs_source)
    gcs_destination = automl_interface.types.GcsDestination(output_uri_prefix=output_uri)
    output_config = automl_interface.types.BatchPredictOutputConfig(
...
if __name__ == '__main__':
    ...
    parser.add_argument('-project_id', default='angular-expanse-455003-v7', type=str)
    ...
    if args.mode in ['all', 'upload', 'imagenet-upload', 'poison_upload']:
        with open('cpoutput.txt', 'rb') as file:
